"""
Experiment script for label-only MIA audit layer.
"""
import argparse
import json
import numpy as np
...
def main():
    parser = argparse.ArgumentParser(description="MIA audit experiment (label-only).")
    ...
    if not args.enable_audit:
        print("Audit disabled. Use --enable_audit to run.")
        return
    ...
    leaf_regions = gen.get_leaf_regions(min_leaf_support=1)
    ...
    audit_report_to_json(report, "audit_report.json")
